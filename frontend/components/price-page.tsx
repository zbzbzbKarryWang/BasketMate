  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (const item of validItems) {
        try {
          const existing = await apiGet<{ id: string } | null>(`/ingredients?name=${encodeURIComponent(item.name)}`)
          ...
          console.error('创建食材失败:', item.name, e)
        }
      }
      ...
      for (const item of validItems) {
        ...
        try {
          ...
          console.error('创建价格失败:', item.name, e)
        }
      }
